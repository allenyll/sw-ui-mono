<template>
  <div class="app-container calendar-list-container">
          ...
          <el-row>
            ...
            <el-col :span="6">
              <el-form-item label="任务分组：">
                <el-input v-model="listQuery.like_job_group" class="input-width" placeholder="手工录入，精确查询"/>
              </el-form-item>
            </el-col>
          </el-row>
        </el-form>
    ...
    <el-table v-loading.body="listLoading" :key="tableKey" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column v-if="show" align="center" label="调度任务主键">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="异常消息" width="500">
        <template slot-scope="scope">
      ...
      <!-- <el-table-column fixed="right" align="center" label="操作">
        <template slot-scope="scope">
          <el-button v-if="jobLogManager_btn_edit" size="small" type="success" @click="handleUpdate(scope.row)">编辑
          </el-button>
          <el-button v-if="jobLogManager_btn_del" size="small" type="danger" @click="handleDelete(scope.row)">删除
          </el-button>
        </template>
      </el-table-column> -->
    </el-table>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="调度任务主键" prop="jobId">
          <el-input v-model="form.jobId" placeholder="请输入调度任务主键"/>
        </el-form-item>
        ...
        <el-form-item label="执行时间" prop="time">
          <el-input v-model="form.time" placeholder="请输入执行时间"/>
        </el-form-item>
...
<script>
import { page, addObj, getObj, delObj, putObj } from '@/api/admin/jobLog/index'
...
export default {
  ...
  data() {
    return {
      ...
      dialogStatus: '',
      jobLogManager_btn_edit: false,
      jobLogManager_btn_del: false,
      jobLogManager_btn_add: false,
      textMap: {
        update: '编辑',
        create: '创建'
      },
  ...
  created() {
    this.getList()
    this.jobLogManager_btn_edit = this.elements['job:jobLog:edit']
    this.jobLogManager_btn_del = this.elements['job:jobLog:delete']
    this.jobLogManager_btn_add = this.elements['job:jobLog:add']
  },
  methods: {
    getList() {
      const jobId = this.$route.query.jobId
      this.listQuery.eq_job_id = jobId
